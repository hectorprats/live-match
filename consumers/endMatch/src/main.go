...
func main() {
	// configContents, err := readFile("consumers/endMatch/src/config.json")
	...
	consume := func(closeFlag chan bool) {
		for m := range msgs {
			...
			var endMatch endMatch
			err := json.Unmarshal(m.Body, &endMatch)
			...
			err = storeEvent(&endMatch, dc)
			fmt.Println("Past store event")
			if err != nil {
				err = errors.Wrap(err, "Unable to store EndMatch event")
				fmt.Println(err)
				continue
			}
...
func storeEvent(endMatch *endMatch, dc *apollo.DatabaseConnection) error {
	...
	payload, err := json.Marshal(endMatch)
	_, err = db.Exec(EndMatch, fmt.Sprintf("%s%s", endMatch.Host, endMatch.Guest), 2, string(payload))
