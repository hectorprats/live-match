...
func main() {
	// configContents, err := readFile("consumers/newOffside/src/config.json")
	...
	consume := func(closeFlag chan bool) {
		for m := range msgs {
			...
			var newOffside newOffside
			err := json.Unmarshal(m.Body, &newOffside)
			...
			err = storeEvent(&newOffside, dc)
			fmt.Println("Past store event")
			if err != nil {
				err = errors.Wrap(err, "Unable to store NewOffside event")
				fmt.Println(err)
				continue
			}
...
func storeEvent(newOffside *newOffside, dc *apollo.DatabaseConnection) error {
	...
	payload, err := json.Marshal(newOffside)
	_, err = db.Exec(NewOffsideSql, fmt.Sprintf("%s%s", newOffside.Host, newOffside.Guest), 3, string(payload))
