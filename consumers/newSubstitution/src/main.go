...
func main() {
	// configContents, err := readFile("consumers/newSubstitution/src/config.json")
	...
	consume := func(closeFlag chan bool) {
		for m := range msgs {
			...
			var newSubstitution newSubstitution
			err := json.Unmarshal(m.Body, &newSubstitution)
			...
			err = storeEvent(&newSubstitution, dc)
			fmt.Println("Past store event")
			if err != nil {
				err = errors.Wrap(err, "Unable to store newSubstitution event")
				fmt.Println(err)
				continue
			}
...
func storeEvent(newSubstitution *newSubstitution, dc *apollo.DatabaseConnection) error {
	...
	payload, err := json.Marshal(newSubstitution)
	_, err = db.Exec(NewSubstitutionSql, fmt.Sprintf("%s%s", newSubstitution.Host, newSubstitution.Guest), 32189, string(payload))
