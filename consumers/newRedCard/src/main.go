...
func main() {
	// configContents, err := readFile("consumers/newRedCard/src/config.json")
	...
	consume := func(closeFlag chan bool) {
		for m := range msgs {
			...
			var newRedCard newRedCard
			err := json.Unmarshal(m.Body, &newRedCard)
			...
			err = storeEvent(&newRedCard, dc)
			fmt.Println("Past store event")
			if err != nil {
				err = errors.Wrap(err, "Unable to store newRedCard event")
				fmt.Println(err)
				continue
			}
...
func storeEvent(newRedCard *newRedCard, dc *apollo.DatabaseConnection) error {
	...
	payload, err := json.Marshal(newRedCard)
	_, err = db.Exec(NewRedCardSql, fmt.Sprintf("%s%s", newRedCard.Host, newRedCard.Guest), 987, string(payload))
